...
<template>
    <div class="min-h-screen bg-slate-50 flex">
        <aside class="fixed left-0 top-0 h-screen w-52 bg-slate-800 shadow-2xl z-50 flex flex-col">
            ...
            <nav class="px-3 py-6 space-y-1.5 flex-1"> <Link 
...
<div v-if="showAddCategoryModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        ...
        <form @submit.prevent="addCategory" class="space-y-4">
            <input v-model="newCategory.reference" placeholder="Référence" class="w-full border p-2 rounded border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" required>
            <input v-model="newCategory.name" placeholder="Nom" class="w-full border p-2 rounded border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" required>
            <textarea v-model="newCategory.description" placeholder="Description" class="w-full border p-2 rounded border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none"></textarea>
            <div class="flex gap-2">
                <button type="button" @click="showAddCategoryModal = false" class="flex-1 px-4 py-2 border border-slate-200 text-slate-600 rounded-lg font-semibold hover:bg-slate-50 transition-colors">Annuler</button>
                <button type="submit" class="flex-1 py-2 bg-teal-600 text-white rounded-lg font-semibold hover:bg-teal-700">Enregistrer</button>
            </div>
        </form>
    </div>
</div>

<div v-if="showEditCategoryModal && editingCategory" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        ...
        <form @submit.prevent="updateCategory" class="space-y-4">
            ...
            <div class="flex gap-2">
                <button type="button" @click="showEditCategoryModal = false" class="flex-1 py-2 border rounded">Annuler</button>
        ...
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
            ...
            <main class="p-8 space-y-6">
                <div class="flex justify-between items-center">
                    ...
                    <button @click="showAddModal = true" class="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-all">
...
<div v-if="showEditModal && editingArticle" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div>
        <label class="block text-sm font-semibold text-slate-700 mb-1">Nom de l'article</label>
        <input v-model="editingArticle.name" type="text" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
    </div>

    <div class="grid grid-cols-2 gap-4">
    <div>
        <label class="block text-sm font-semibold text-slate-700 mb-1">Catégorie</label>
        <input v-model="editingArticle.category" type="text" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm">
    </div>
    <div>
        <label class="block text-sm font-semibold text-slate-700 mb-1">Statut</label>
        <input v-model="editingArticle.status" type="text" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm">
    </div>
</div>
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input v-model="editingArticle.stock" type="number" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
        </div>
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">Seuil (Lieu)</label>
            <input v-model="editingArticle.threshold" type="text" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
        </div>
    </div>

    <div class="pt-4 flex gap-3">
        <button type="button" @click="showEditModal = false" class="flex-1 px-4 py-2 border text-slate-600 rounded-lg hover:bg-slate-50">Annuler</button>
        <button type="submit" class="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg font-semibold hover:bg-teal-700">Mettre à jour</button>
    </div>
</form>
    </div>
</div>
<div v-if="showAddModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
        ...
        <form @submit.prevent="addArticle" class="p-6 space-y-4">
            <div>
                ...
                <input v-model="newArticle.reference" type="text" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none">
            </div>
            
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">Nom de l'article</label>
                <input v-model="newArticle.name" type="text" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none">
            </div>

            <div class="grid grid-cols-2 gap-4">
    <div>
        <label class="block text-sm font-semibold text-slate-700 mb-1">Catégorie</label>
        <input v-model="newArticle.category" type="text" placeholder="Ex: Fournitures" class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-sm">
    </div>
    <div>
        <label class="block text-sm font-semibold text-slate-700 mb-1">Seuil (Lieu)</label>
        <input v-model="newArticle.threshold" type="text" placeholder="Ex: Campus A" class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-sm">
    </div>
    <div>
        <label class="block text-sm font-semibold text-slate-700 mb-1">Status</label>
        <input v-model="newArticle.status" type="text" placeholder="Ex: En stock" class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-sm">
    </div>
    <div>
        <label class="block text-sm font-semibold text-slate-700 mb-1">Stock</label>
        <input v-model="newArticle.stock" type="number" class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none">
    </div>
</div>

            <div class="pt-4 flex gap-3">
                <button type="button" @click="showAddModal = false" class="flex-1 px-4 py-2 border border-slate-200 text-slate-600 rounded-lg font-semibold hover:bg-slate-50 transition-colors">
                    Annuler
                </button>
                <button type="submit" class="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg font-semibold hover:bg-teal-700 transition-colors">
                    Enregistrer
                </button>
            </div>
        </form>
                ...
                <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-wrap gap-4 items-center">
                    <div class="relative flex-1 min-w-[200px]">
                        ...
                        <input type="text" placeholder="Rechercher un article..." class="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none">
                    </div>
    ...
    <tr v-for="art in articles" :key="art.id" class="hover:bg-slate-50 transition-colors">
        ...
        <td class="px-6 py-4">
            <div class="flex justify-center gap-3">
                <button @click="openEditModal(art)" class="p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                    </svg>
                </button>
                <button @click="deleteArticle(art.id)" class="p-1.5 text-red-400 hover:bg-red-50 rounded-lg transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <button @click="showAddCategoryModal = true" class="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-all">
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
    Nouvelle Catégorie
</button>
                </div>
                ...
                <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-wrap gap-4 items-center">
                    <div class="relative flex-1 min-w-[200px]">
                        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                        <input type="text" placeholder="Rechercher une catégorie..." class="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none">
                    </div>
    ...
    <td class="px-6 py-4">
        <div class="flex justify-center gap-3">
            <button @click="openEditCategoryModal(cat)" class="p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
            </button>
            <button @click="deleteCategory(cat.id)" class="p-1.5 text-red-400 hover:bg-red-50 rounded-lg transition-colors">
