                    ...
                    <form @submit.prevent="saveUser" class="p-6 space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Email
                                professionnel</label>
                            <input v-model="userForm.email" type="email" placeholder="m.ndiaye@stockflow.sn" required
                                class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                    ...
                    <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-slate-50/80 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                <span class="text-sm font-bold text-slate-800">{{ user.service?.ser_nom || 'SansService'
                                    }}</span>
            ...
            <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex flex-col items-center gap-2">
                <div class="flex items-center gap-1">
                    <Link preserve-scroll v-for="(link, k) in props.users.links" :key="k" :href="link.url || '#'"
