    ...
    <main class="p-8 space-y-6">
        ...
        <Teleport to="body">
            <div v-if="showAddMouvementModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
                <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" @click="showAddMouvementModal = false">
                </div>
                    ...
                    <form @submit.prevent="addMouvement" class="p-6 space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <div v-if="movementForm.errors.mvs_date_mouvement" class="text-red-500 text-xs mt-1">{{
                                    movementForm.errors.mvs_date_mouvement
                                    }}</div>
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div v-if="movementForm.mvs_type === 'TRANSFER'">
                                ...
                                <div v-if="movementForm.errors.mvs_ent_dest_id" class="text-red-500 text-xs mt-1">{{
                                    movementForm.errors.mvs_ent_dest_id }}
                                </div>
                            </div>
            ...
            <div class="bg-white rounded-xl shadow-md border border-slate-100 overflow-hidden">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <tr v-for="movement in props.stockMovements.data" :key="movement.mvs_id"
                                    ...
                                    <td class="px-6 py-4 text-sm font-bold" :class="{
                                        'text-emerald-600': movement.mvs_type === 'IN',
                                        'text-red-600': movement.mvs_type === 'OUT',
                                        'text-blue-600': movement.mvs_type === 'TRANSFER',
                                        'text-amber-600': movement.mvs_type === 'ADJUST',
                                    }">
                ...
                <div v-if="props.stockMovements.links && props.stockMovements.links.length > 3"
                    ...
                    <div class="flex items-center gap-2">
                        <Link preserve-scroll v-for="(link, k) in props.stockMovements.links" :key="k"
                            :href="link.url || '#'" v-html="link.label"
                            class="px-3 py-1 text-sm rounded flex items-center justify-center transition-all" :class="{
        ...
        <!-- Entrepots Section -->
        <div v-show="activeTab === 'entrepots'"
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="ent in filteredEntrepots" :key="ent.ent_id"
                        ...
                        <td class="px-6 py-4 text-sm text-slate-500">{{ new
                            Date(ent.ent_created_at).toLocaleDateString('fr-FR') }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <button @click="openEditEntrepotModal(ent)"
                                ...
                                <button @click="deleteEntrepot(ent.ent_id)"
                                    class="p-1.5 text-red-400 hover:bg-red-50 rounded-lg transition-colors"
                                    title="Supprimer">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex flex-col items-center gap-2">
                <div class="flex items-center gap-1">
                    <Link preserve-scroll v-for="(link, k) in (props.warehouses_paginated?.links ?? [])" :key="k"
                ...
                <span class="text-xs text-slate-400">{{ props.warehouses_paginated?.from ?? 0 }}–{{
                    props.warehouses_paginated?.to ?? 0 }} sur {{ props.warehouses_paginated?.total ?? 0 }}
                    entrepôts</span>
            </div>
        </div>
