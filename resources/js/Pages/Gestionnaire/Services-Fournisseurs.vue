        ...
        <div class="ml-52 flex-1">
            <header class="bg-white border-b border-slate-200 sticky top-0 z-10 px-8 py-4 flex justify-between">
                <div class="flex items-center gap-4 text-slate-500">
                    <Link :href="route('gestionnaire.utilisateurs.index')"
            ...
            <main class="p-8 space-y-6">
                ...
                <div v-if="activeTab === 'services'"
                        ...
                        <tbody class="divide-y divide-slate-50">
                            <tr v-for="s in services" :key="s.id" class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm"><span
                                        class="bg-slate-100 px-2 py-1 rounded text-slate-500">{{ s.etage }}</span></td>
                    ...
                    <div
                        class="px-8 py-5 flex flex-col items-center justify-center gap-2 bg-white border-t border-slate-100 w-full">
                        <div class="flex items-center gap-2">
                            <button class="text-slate-400 hover:text-teal-600 font-bold">&lt;</button>
                            <button
                                class="w-8 h-8 bg-teal-600 text-white rounded flex items-center justify-center text-sm font-bold shadow-sm">
                                1
                            </button>
                            <button class="text-slate-400 hover:text-teal-600 font-bold">&gt;</button>
                        </div>
                        <span class="text-xs text-slate-500 font-medium tracking-wide">1-3 Sur 3</span>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            <tr v-for="f in fournisseurs" :key="f.id" class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-2 py-1 rounded-md text-[10px] font-bold bg-blue-50 text-blue-600 border border-blue-100 uppercase">{{
                                            f.categorie }}</span>
                                </td>
                    ...
                    <form @submit.prevent="saveEntry" class="p-6 space-y-4">
                        ...
                        <div v-if="activeTab === 'services'" class="space-y-4">
                            <div>
                                ...
                                <input v-model="formData.responsable" type="text"
                                    class="w-full px-4 py-2 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-teal-500">
                            </div>

                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-semibold text-slate-700 mb-1">Localisation
                                        (Étage)</label>
                                    <input v-model="formData.etage" type="text" placeholder="ex: 2ème étage"
                                        class="w-full px-4 py-2 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-teal-500">
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold text-slate-700 mb-1">Effectif</label>
                                    <input v-model="formData.effectif" type="number"
                                        class="w-full px-4 py-2 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-teal-500">
                                </div>
                            </div>
                        </div>
                        <div v-if="activeTab === 'fournisseurs'" class="space-y-4">
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 mb-1">Catégorie
                                    d'activité</label>
                                <input v-model="formData.categorie" type="text"
