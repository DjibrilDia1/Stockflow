...
const applyFilters = () => {
    ...
    timeout = setTimeout(() => {
        router.get(route('responsable.rapports.index'), {
            tab: 'movements',
            ...filterForm.value
        }, {
            preserveState: true,
...
const changeTab = (tab) => {
    ...
    router.get(route('responsable.rapports.index'), { tab }, {
        preserveState: true,
        preserveScroll: true
    });
};
...
const exportExcel = () => {
    ...
    window.location.href = route('responsable.rapports.export') + '?' + params;
};
        ...
        <div class="ml-52 flex-1">
            <header class="bg-white border-b border-slate-200 sticky top-0 z-10 px-8 py-4 flex items-center justify-between">
                <div class="flex items-center gap-4 text-slate-500">
                    <span class="font-medium text-lg text-slate-800">Rapports de Performance</span>
                </div>
            ...
            <main class="p-8">
                ...
                <div v-if="currentTab === 'low-stock'" class="space-y-6">
                    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50"><h3 class="text-lg font-semibold text-slate-800">État des Alertes</h3></div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-left">
                                <thead>
                                    <tr class="bg-slate-50/30 text-xs font-bold text-slate-500 uppercase"><th class="px-6 py-4">Référence</th><th class="px-6 py-4">Article</th><th class="px-6 py-4">Entrepôt</th><th class="px-6 py-4">Stock Actuel</th><th class="px-6 py-4">Seuil</th><th class="px-6 py-4">Sécurité</th><th class="px-6 py-4">Statut</th></tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100">
                                    ...
                                    <tr v-if="lowStockData.length === 0"><td colspan="7" class="px-6 py-12 text-center text-slate-500">Aucune alerte en cours.</td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                ...
                <div v-if="currentTab === 'movements'" class="space-y-6">
                    ...
                    <div class="flex justify-end"><button @click="exportExcel" class="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all shadow-sm">Télécharger Rapport Excel</button></div>
                    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                        <div class="overflow-x-auto">
                            <table class="w-full text-left">
                        ...
                        <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                            <div class="text-sm text-slate-500">Total: {{ movementsData.total }} mouvements</div>
                ...
                <div v-if="currentTab === 'kpi'" class="space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100"><p class="text-slate-500 text-sm font-medium">Alertes Stock</p><h4 class="text-3xl font-bold text-slate-800 mt-2">{{ kpiData.underThreshold }}</h4></div>
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100"><p class="text-slate-500 text-sm font-medium">Activités du mois</p><h4 class="text-3xl font-bold text-slate-800 mt-2">{{ kpiData.movementsThisMonth }}</h4></div>
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100"><p class="text-slate-500 text-sm font-medium">Top Consommation</p><h4 class="text-lg font-bold text-slate-800 mt-2 truncate">{{ kpiData.mostConsumedItem?.name || '-' }}</h4><div class="mt-1 text-sm text-slate-500">Total: {{ Math.abs(kpiData.mostConsumedItem?.quantity || 0) }}</div></div>
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100"><p class="text-slate-500 text-sm font-medium">Catalogue Articles</p><h4 class="text-3xl font-bold text-slate-800 mt-2">{{ kpiData.totalArticles }}</h4></div>
                    </div>

                    <!-- Charts Section -->
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
                        <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                            <h3 class="text-lg font-semibold text-slate-800 mb-4">Analyse Comparative</h3>
                            <div class="h-64">
                                <Bar :data="barChartData" :options="barChartOptions" />
                            </div>
                        </div>
                        <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                            <h3 class="text-lg font-semibold text-slate-800 mb-4">Répartition Globale du Stock</h3>
                            <div class="h-64">
