...
class DemandeSortie extends Model
{
    ...
    /**
     * Approve the request and update stock.
     */
    public function approve(): bool
    {
        ...
        foreach ($this->lines as $line) {
            $stock = StockArticle::where('sta_art_id', $line->lds_art_id)
    ...
    /**
     * Create a complete request with its first line.
     */
    public static function createWithLine(array $data, int $userId): self
    {
        $user = User::find($userId);
        $serviceId = $user->ser_id ?? Service::first()->ser_id;

        $demande = self::create([
            'dso_ser_id' => $serviceId,
            'dso_demandeur_id' => $userId,
            'dso_statut' => 'DRAFT',
        ]);

        $demande->lines()->create([
            'lds_art_id' => $data['article_id'],
            'lds_ent_id' => $data['entrepot_id'],
            'lds_qte_demandee' => $data['quantite'],
            'lds_note' => $data['motif'] ?? null,
        ]);
